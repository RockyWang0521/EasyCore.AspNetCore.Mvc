...
{
    /// <summary>
    /// Factory for creating remote API clients whose base address is resolved via Consul.
    /// </summary>
    public static class RemoteApiConsulClientFactory
    {
        ...
        /// <param name="httpClient">The HTTP client (typically with a Consul resolving handler).</param>
            ...
            Type iface)
        {
            if (!iface.IsInterface)
                throw new ArgumentException("interfaceType must be an interface", nameof(iface));

            var method = typeof(RemoteApiConsulClientFactory)
